...
namespace RecoQQ {
  void iniBranches(TChain* fChain)
  {
    ...
    fChain->SetBranchStatus("Reco_QQ_mumi_TMOneStaTight",1);
  ...
  Bool_t isTrkMuonInAccept2015 (TLorentzVector* Muon) 
  {
    return (fabs(Muon->Eta()) < 2.4 && 
	    ( (fabs(Muon->Eta()) < 1.2 && Muon->Pt() >= 3.3 ) 
	      || (1.2 <= fabs(Muon->Eta()) && fabs(Muon->Eta()) < 2.1 && Muon->Pt() >= 3.93-1.11*fabs(Muon->Eta() )) 
	      || (2.1 <= fabs(Muon->Eta()) && Muon->Pt() >= 1.3 )) );
  };

  Bool_t areTrkMuonsInAcceptance2015 (Int_t iRecoQQ)
  {
    TLorentzVector *RecoQQmupl = (TLorentzVector*) Reco_QQ_mupl_4mom->At(iRecoQQ);
    TLorentzVector *RecoQQmumi = (TLorentzVector*) Reco_QQ_mumi_4mom->At(iRecoQQ);
    return ( isTrkMuonInAccept2015(RecoQQmupl) && isTrkMuonInAccept2015(RecoQQmumi) );
  }; 
  

  Bool_t passTrkQualityCuts2015 (Int_t iRecoQQ) 
  {
    Bool_t cond = true;
